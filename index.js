...
function getFullNames(runners) {
  const firstLast = [];
  runners.forEach(runner => firstLast.push(`${runner.last_name}, ${runner.first_name}`)
  );
  return firstLast;
}
...
function firstNamesAllCaps(runners) {
  const yelling =[];
  runners.forEach(runner => yelling.push(`${runner.first_name.toUpperCase()}`))
  return yelling
}
...
function tallyUpDonations(runners) {
  return runners.reduce((total, currentValue) => total += currentValue.donation, 0)
